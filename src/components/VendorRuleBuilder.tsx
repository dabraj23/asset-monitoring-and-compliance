...
export function VendorRuleBuilder() {
  ...
  const editRule = (rule: typeof configuration.draftRules[number]) => {
    ...
    setForm({ name: rule.name, description: rule.description, source: rule.regulatorySource, scope: rule.scope, categoryId: rule.categoryIds[0] || '', activityTag: rule.activityTagsAny[0] || '', personnelRole: rule.personnelRolesAny[0] || '', documentType: legacyAgreement ? 'CONTRACT_REFERENCE' : rule.documentType, requiredFields: legacyAgreement ? '' : rule.requiredFields.join(', '), connector: legacyAgreement ? 'CONTRACT_STATUS' : rule.connector, blocking: rule.blocking, active: rule.active, warningDays: rule.expiryWarningDays, slaDays: rule.followUpSlaDays, owner: rule.escalationOwner, steps: rule.steps });
  };
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[0.75fr_1.25fr]">
      ...
      <section className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 max-h-[720px] space-y-3 overflow-y-auto pr-1">{configuration.draftRules.map(rule => <div key={rule.id} className="rounded-xl border border-gray-100 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="font-bold text-gray-900">{rule.name}</div><div className="mt-1 text-sm text-gray-500">{rule.description}</div></div><div className="flex items-center gap-2"><button onClick={() => editRule(rule)} className="text-xs font-semibold text-blue-700">Edit</button><div className={`rounded-full px-2.5 py-1 text-xs font-bold ${rule.blocking ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'}`}>{rule.blocking ? 'Blocking' : 'Warning'}</div></div></div><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded bg-gray-100 px-2 py-1">{rule.id === 'agreement' && rule.connector === 'DOCUMENT_ONLY' && rule.documentType === 'VENDOR_AGREEMENT' ? 'Contract Management reference (legacy)' : rule.documentType.replace(/_/g, ' ')}</span><span className="rounded bg-blue-50 px-2 py-1 text-blue-700">{rule.id === 'agreement' && rule.connector === 'DOCUMENT_ONLY' && rule.documentType === 'VENDOR_AGREEMENT' ? 'Contract status' : rule.connector.replace(/_/g, ' ')}</span><span className="rounded bg-gray-100 px-2 py-1">{rule.steps.length} workflow steps</span></div></div>)}</div>
      </section>
    </div>
  </div>;
}
